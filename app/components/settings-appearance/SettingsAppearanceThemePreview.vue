...
const selectedClass = 'relative before:inset-0 before:rounded before:absolute before:border-2 before:border-[#00fd0d]'
</script>

<template>
	<div class="cursor-pointer relative flex flex-col gap-2">
		<div v-if="selected" class="absolute z-10 -top-[6%] -right-[6%] w-6 h-6 bg-[#00fd0d] rounded-full flex items-center justify-center">
			<Icon class="h-5 text-white" name="material-symbols-check-rounded"/>
		</div>
	<div :class="[backgroundColor, selected ? selectedClass : '']" class="w-36 flex flex-col gap-2 rounded p-2">

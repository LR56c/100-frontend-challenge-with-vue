...
<div class="min-w-64 max-w-64 flex flex-col p-4 rounded-2xl shadow-lg gap-4 bg-white">
	<div class="flex justify-between">
		<span
			:class="selectedColor"
			class="text-white text-xs uppercase font-medium px-2.5 py-0.5 rounded-full">{{ priority }}</span>
		<div class="text-sm text-gray-400">{{ df.format(date) }}</div>
	</div>
	<img v-if="img" class="rounded-2xl" :src="img"/>
	<div class="font-bold">
		{{ title }}
	</div>
	<div>
	...
	<task-avatar class="z-10" v-for="personImg in personsImages" :img="personImg"></task-avatar>
		<task-avatar @newClick="addPerson" class="z-0" dotted></task-avatar>
	</div>
</div>
</template>
